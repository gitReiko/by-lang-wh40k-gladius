...
- Retain existing translations that remain applicable. Translate new entries and review changed entries using `gladius-translate`, consulting the current root-level glossaries: `слоўнік gladius.txt` for game and interface terms and `слоўнік warhammer 40k.txt` for Warhammer universe terms. Add clear new equivalents to the appropriate glossary after checking both files for existing entries. Do not replace an entire Belarusian file with an English template.
